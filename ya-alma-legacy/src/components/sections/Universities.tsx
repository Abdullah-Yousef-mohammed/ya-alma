...
export default function Universities() {
  ...
  return (
    <section id="universities" className="py-24 bg-white relative overflow-hidden" dir={language === "ar" ? "rtl" : "ltr"}>
      {/* Decorative BG */}
        ...
        >
          <div className="embla__container gap-6 md:gap-8 pb-10">
            {universities.map((uni, index) => (
              ...
              >
                <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgb(0,0,0,0.1)] border border-gray-100 h-full flex flex-col group transition-all duration-500 hover:-translate-y-2">
                  <div className="h-32 bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-[1.5rem] flex items-center justify-center mb-6 border border-gray-200/50 text-3xl font-bold text-gray-300 group-hover:from-blue-50 group-hover:to-transparent group-hover:text-blue-500 transition-all duration-500">
                    {uni.logoUrl || "Logo"}
                  </div>
                  <h3 className="text-xl font-bold text-gray-900 mb-3 truncate group-hover:text-[var(--color-brand-navy)] transition-colors" title={t_dyn(uni.name, uni.nameAr)}>{t_dyn(uni.name, uni.nameAr)}</h3>
                  ...
                  <a href={`/universities`} className="mt-auto inline-flex items-center justify-between text-sm font-bold text-[var(--color-brand-navy)] hover:text-[var(--color-brand-gold)] transition-colors group/btn">
                    <span className="bg-gray-50 px-4 py-2 rounded-xl group-hover/btn:bg-[var(--color-brand-gold)]/10 transition-colors">
                      {t.universities.btn} 
                    </span>
