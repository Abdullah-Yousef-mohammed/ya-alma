...
export default function EditSpecializationVisual() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://yaalmalegacy.com/api"}/specializations/${id}`).then(r => r.json()),
      fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://yaalmalegacy.com/api"}/universities`).then(r => r.json()),
    ])
    .then(([spData, unis]) => {
      // Parse JSON
      const parsedData = {
  ...
  const handleSave = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://yaalmalegacy.com/api"}/specializations/${id}`, {
      method: "PUT",
