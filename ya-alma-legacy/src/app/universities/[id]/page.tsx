          ...
          {[
            ...
            <React.Fragment key={idx}>
              <div className="flex flex-col items-center">
                <div className="w-[72px] md:w-[130px] h-[80px] md:h-[140px] bg-white/5 backdrop-blur-xl border border-[#c6a345]/30 rounded-2xl md:rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(198,163,69,0.3)] flex items-center justify-center relative overflow-hidden group hover:bg-white/10 transition-all duration-300">
    ...
    const fetchData = async () => {
      try {
        const [uniRes, courseRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api"}/universities/${params.id}`),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api"}/courses/university/${params.id}`)
        ]);
      ...
      <section className="relative pt-32 pb-20 bg-[var(--color-brand-navy)] text-white overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10 flex flex-col">
          <Link href="/universities" className="inline-flex items-center text-gray-300 hover:text-white font-medium transition-colors mb-8">
            ...
            {t_dyn("Back to Directory", "العودة إلى الجامعات")}
          </Link>
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className={`${isRtl ? 'text-center md:text-right' : 'text-center md:text-left'} flex-grow`}>
              <div className={`flex flex-wrap items-center justify-center ${isRtl ? 'md:justify-end' : 'md:justify-start'} gap-3 mb-4`}>
                <span className="bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase border border-white/20">
                  {university.isPrivate 
                    ...
                    : t_dyn("Public Institution", "مؤسسة حكومية", "公立机构")}
                </span>
                {university.freeOfferLetter && (
                  <span className="bg-green-500/90 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase shadow-lg border border-green-400/50">
                    {t_dyn("Free Offer Letter", "رسالة قبول مجانية", "免费录取通知书")}
                  </span>
                )}
              </div>
              <h1 className="text-4xl md:text-6xl font-black mb-2 tracking-tight">{t_dyn(university.name, university.nameAr, university.nameZh || university.name)}</h1>
              <h2 className="text-2xl text-[var(--color-brand-gold)] mb-6 font-medium">{t_dyn(university.nameAr, university.name, university.name)}</h2>
              <div className={`flex flex-wrap items-center justify-center ${isRtl ? 'md:justify-end' : 'md:justify-start'} text-gray-300 font-medium gap-6 text-sm`}>
                <span className="flex items-center gap-2"><MapPin size={18} className="text-[var(--color-brand-gold)]" /> {t_dyn(university.location, university.locationAr, university.locationZh || university.location)}</span>
                <span className="flex items-center gap-2"><BookOpen size={18} className="text-[var(--color-brand-gold)]" /> {totalPrograms} {t_dyn("Accredited Programs", "برامج معتمدة", "认证项目")}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex items-center gap-1 py-3 whitespace-nowrap overflow-x-auto no-scrollbar">
            {[
              ...
              { id: 'programs', label: t_dyn("Programs", "البرامج", "学术项目"), icon: <GraduationCap size={16}/> },
        ...
        <div className="w-full xl:w-[68%] space-y-12">
          ...
          <section id="overview" className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 md:p-12 scroll-mt-40 transition-all duration-500 hover:shadow-xl hover:shadow-gray-200/50">
            <div className="flex items-center gap-4 mb-8 text-[var(--color-brand-navy)]">
              ...
              <h3 className="text-3xl font-black">{t_dyn("University Overview", "نظرة عامة على الجامعة")}</h3>
            </div>
            <div className="prose prose-lg max-w-none text-gray-600 leading-[2.2] text-lg">
              <div className="relative">
                ...
                <p className="whitespace-pre-line">{t_dyn(university.aboutEn, university.aboutAr, university.aboutZh || university.aboutEn)}</p>
              </div>
            </div>
          </section>
          ...
          <section id="tour" className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 md:p-12 scroll-mt-40 transition-all duration-500 hover:shadow-xl hover:shadow-gray-200/50">
            ...
            <div className="aspect-video rounded-[2.5rem] overflow-hidden bg-gray-900 relative shadow-2xl border-4 border-white">
              {isVideoPlaying ? (
                <iframe
                  src={`https://www.youtube.com/embed/${t_dyn(university.videoUrl, university.videoUrlAr || university.videoUrl, university.videoUrlZh || university.videoUrl)}?autoplay=1&controls=1&rel=0`}
                ...
                >
                  <img 
                    src={`https://img.youtube.com/vi/${t_dyn(university.videoUrl, university.videoUrlAr || university.videoUrl, university.videoUrlZh || university.videoUrl)}/maxresdefault.jpg`} 
                    ...
                    onError={(e) => { e.currentTarget.src = `https://img.youtube.com/vi/${t_dyn(university.videoUrl, university.videoUrlAr || university.videoUrl, university.videoUrlZh || university.videoUrl)}/hqdefault.jpg`; }}
                  />
          ...
          {/* Programs Section */}
          <section id="programs" className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 md:p-12 overflow-hidden scroll-mt-40 transition-all duration-500 hover:shadow-xl hover:shadow-gray-200/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
              <div className="flex items-center gap-4 text-[var(--color-brand-navy)]">
                <div className="w-12 h-12 bg-[var(--color-brand-gold)]/10 rounded-2xl flex items-center justify-center text-[var(--color-brand-gold)]">
                  <GraduationCap size={28} />
                </div>
                <h3 className="text-3xl font-black">{t_dyn("Available Programs", "البرامج والرسوم")}</h3>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {Object.entries(facultyGroups).map(([key, faculty]: [string, any]) => {
                ...
                return (
                  <div key={key} className={`rounded-[2rem] overflow-hidden border-2 transition-all duration-500 ${isOpen ? 'border-[var(--color-brand-navy)]/40 shadow-xl' : 'border-gray-100 bg-gray-50/30'}`}>
                    {/* Accordion Header */}
                    <button
                      onClick={() => toggleFaculty(key)}
                      className={`w-full flex items-center justify-between p-7 text-left transition-all duration-500 ${isOpen ? 'bg-[var(--color-brand-navy)] text-white' : 'hover:bg-gray-100/50'}`}
                    >
                      <div className="flex items-center gap-6">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl transition-all duration-500 ${isOpen ? 'bg-white/20 text-white' : 'bg-[var(--color-brand-gold)]/10 text-[var(--color-brand-gold)]'}`}>
                          {faculty.programs.length}
                        </div>
                        <div className={isRtl ? 'text-right' : 'text-left'}>
                          <h4 className="font-black text-xl leading-tight">{t_dyn(faculty.name, faculty.nameAr, faculty.nameZh || faculty.name)}</h4>
                          <p className={`text-xs mt-1 font-bold ${isOpen ? 'text-blue-200' : 'text-gray-400'}`}>{faculty.programs.length} {t_dyn('Programs Available', 'برنامج متاح')}</p>
                        </div>
                      </div>
                    ...
                    <div className={`overflow-hidden transition-all duration-700 ease-in-out ${isOpen ? 'max-h-[3000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                      <div className="p-2 md:p-8 bg-white overflow-x-auto">
                        <table className={`w-full ${isRtl ? 'text-right' : 'text-left'} border-collapse min-w-[700px]`}>
                          <thead>
                            <tr className="text-gray-400 font-bold text-[11px] uppercase tracking-[0.2em] border-b border-gray-100">
                              <th className="py-6 px-4">{t_dyn("Program", "البرنامج", "课程")}</th>
                              <th className="py-6 px-4">{t_dyn("Degree", "الدرجة", "学位")}</th>
                              <th className="py-6 px-4">{t_dyn("Duration", "المدة", "学制")}</th>
                              <th className="py-6 px-4">{t_dyn("Fee/Year", "الرسوم", "学费/年")}</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-50">
                            {faculty.programs.map((prog: any) => (
                              <tr key={prog.id} className="group/row hover:bg-gray-50/80 transition-all">
                                <td className="py-7 px-4">
                                  <p className="font-black text-gray-900 text-lg mb-1">{t_dyn(prog.titleEn, prog.titleAr, prog.titleZh || prog.titleEn)}</p>
                                  <div className="flex flex-wrap gap-1.5 text-[10px] font-black tracking-widest uppercase mt-2">
                                    {(t_dyn(prog.intakes, prog.intakesAr, prog.intakesZh || prog.intakes) || "").split(",").map((i: string, idx: number) => {
                                ...
                                <td className="py-7 px-4">
                                  <span className="text-[10px] font-black border-2 border-gray-100 text-gray-500 px-3 py-1.5 rounded-full uppercase tracking-widest">{t_dyn(prog.level, prog.levelAr, prog.levelZh || prog.level)}</span>
                                </td>
                                <td className="py-7 px-4">
                                  <div className="flex items-center gap-2 text-gray-600 font-bold text-sm">
                                    <Calendar size={14} className="text-[var(--color-brand-gold)]" />
                                    {t_dyn(prog.duration, prog.durationAr, prog.durationZh || prog.duration)}
                                  </div>
                                </td>
                                <td className="py-7 px-4">
                                  <span className="text-2xl font-black text-[var(--color-brand-navy)]">{formatPrice(prog.feeMyr)}</span>
                                  <span className="text-[10px] text-gray-400 font-bold block">{t_dyn("per academic year", "لكل سنة أكاديمية", "每学年")}</span>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                  <div>
                    ...
                    <p className="text-gray-500 text-lg leading-relaxed mb-8">
                       {t_dyn(
                         university.scholarshipDescEn || "Financial aid packages are available up to 50% for high-achieving students.", 
                         university.scholarshipDescAr || "تتوفر حزم مساعدات مالية تصل إلى 50%.",
                         university.scholarshipDescZh
                       )}
                    </p>
             ...
             <div className="grid md:grid-cols-2 gap-10">
               <div className="bg-gray-50/50 p-10 rounded-[2.5rem] border border-gray-100">
                 <h4 className="text-2xl font-black text-[var(--color-brand-navy)] mb-8 flex items-center gap-3">
                   ...
                   {t_dyn("Bachelor Requirements", "متطلبات البكالوريوس")}
                 </h4>
               ...
               <div className="bg-gray-50/50 p-10 rounded-[2.5rem] border border-gray-100">
                 <h4 className="text-2xl font-black text-[var(--color-brand-navy)] mb-8 flex items-center gap-3">
                   ...
                   {t_dyn("Master/PhD Entry", "الدراسات العليا")}
                 </h4>
                 <ul className="space-y-6">
             ...
             <div className="flex flex-col md:flex-row gap-10">
               <div className="md:w-1/3">
                  <div className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100 h-full">
                    ...
                    <h4 className="text-2xl font-black text-[var(--color-brand-navy)] mb-6">{t_dyn(university.location, university.locationAr, university.locationZh || university.location)}</h4>
               ...
               <div className="md:w-2/3 aspect-video rounded-[3rem] overflow-hidden bg-gray-100 border-4 border-white shadow-2xl relative">
                  <div 
                    ...
                    style={{ backgroundImage: `url('${university.locationMapUrl || "https://images.unsplash.com/photo-1524661135-423995f22d0b?q=80&w=1500"}')` }}
                  ></div>
               </div>
             </div>
          </section>
        ...
        <div className="w-full xl:w-[35%] space-y-5 xl:sticky xl:top-[160px] z-30">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="bg-gradient-to-r from-[var(--color-brand-navy)] to-[#1a2f6b] px-7 py-5">
                <h3 className="text-white font-black text-lg">{t_dyn('University at a Glance', 'نظرة سريعة على الجامعة', '大学概览')}</h3>
              </div>
              <div className="divide-y divide-gray-100">
                {[
                  { icon: <Globe size={18}/>, label: t_dyn('Type', 'النوع', '类型'), value: university.isPrivate ? t_dyn('Private University', 'جامعة خاصة', '私立大学') : t_dyn('Public University', 'جامعة حكومية', '公立大学') },
                  { icon: <MapPin size={18}/>, label: t_dyn('Location', 'الموقع', '位置'), value: t_dyn(university.location, university.locationAr, university.locationZh || university.location) },
                  { icon: <Award size={18}/>, label: t_dyn('Ranking', 'التصنيف', '排名'), value: (language === 'zh' ? university.rankingsZh?.[0] : language === 'ar' ? university.rankingsAr?.[0] : university.rankings?.[0]) || t_dyn('Top Tier', 'تصنيف عالمي', '顶尖排名') },
